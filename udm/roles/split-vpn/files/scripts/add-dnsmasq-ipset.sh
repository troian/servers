...
write_jobs_to_cronfile() {
	...
	echo "$jobs_map" | jq -r 'keys[]' | while read -r cron_time; do
		commands=$(echo "$jobs_map" |
			jq -r '[
					([.["'"${cron_time}"'"]["prefixes"][] | "'"${ipset_bin}"' flush \(.)FORCED4; '"${ipset_bin}"' flush \(.)FORCED6; '"${ipset_bin}"' flush \(.)EXEMPT4; '"${ipset_bin}"' flush \(.)EXEMPT6"] | join("; ")),
					(.["'"${cron_time}"'"]["commands"] | join("; "))
					] | join("; ")
					')
		echo "${cron_time} ${username}${commands}" >> "$cron_file"
	done
...
write_ipsets_to_config() {
	...
	echo "$ipsets_map" | jq -r 'keys[]' | while read -r config_location; do
		config="${config_location}/ipsets-$vpn_name.conf"
...
for conf in "$(dirname "$0")/wireguard/${vpn_name}"/ipsets/*.conf; do
	echo "add-dnsmasq-ipsets: Loading configuration from ${conf}"
	unset FORCED_DOMAINS EXEMPT_DOMAINS FORWARD_SERVERS CRON_TIME
